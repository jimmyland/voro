    ...
    this.get_binary_raw_buffer = function(filter_unused) {
        ...
        if (sym) {
            ...
            for (i=0; i<symInfo.params.length; i++) {
                view.setFloat32(sym_start+8+4*i, symInfo.params[i], true);
            }
        } else {
    ...
    this.create_from_raw_buffer = function(scene, buffer) {
        ...
        if (version > 1) { // includes symmetry
            ...
            if (sym !== 0) {
                ...
                {
                    console.log("WARNING: save file has incorrect data due to now-fixed bug; resave to fix the file.");
                    // hack to work around earlier bug in save where nparams was overwritten w/ the param data
                    sym_param = view.getFloat32(cur_pos - 4, true);
                }
                if (nparams === 1) {
